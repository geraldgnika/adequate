<?php
/**
 * Class that holds static methods, used in validating a form
 */
class FormValidator {
	/**
	 ...
	 */
	public static function formIsSet($submit, $fields = array(), $errorRoute, $csrf = false) {
		if ($csrf) {
			$csrf_pass = Form::check_token($_POST['csrf_token']);

			if (!isset($_POST['csrf_token']) && !$csrf_pass) {
				$error = rawurlencode(CSRF_ERROR);
